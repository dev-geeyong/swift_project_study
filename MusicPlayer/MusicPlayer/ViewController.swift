...
//        self.addViewsWithCode()
        ...
        /* 코드로 뷰 그리기
        let frame = CGRect(x: 60, y: 100, width: 240, height: 500)
    ...
    // MARK: 초기설정
    func initializePlayer(){ //player 초기설정
        guard let soundAsset: NSDataAsset = NSDataAsset(name: "sound") else{
            print("음원 파일 에셋 가져올 수 없습니다")
            return
        }
        do{
            try self.player = AVAudioPlayer(data: soundAsset.data) //음악 넣기
            self.player.delegate = self
        } catch let error as NSError{
            print("플레이어 초기화 실패")
            print("코드 : \(error.code), 메세지 : \(error.localizedDescription)")
        }
        self.progressSlider.maximumValue = Float(self.player.duration) //슬라이더 최대값, 최소값, 현재값 설정
        self.progressSlider.minimumValue = 0
        self.progressSlider.value = Float(self.player.currentTime)
    }
    func updateTimeLabelText(time: TimeInterval) { //시간업데이트
        ...
        self.timeLabel.text = timeText //텍스트 레이블에 시간값 설정
        
    }
    func makeAndFireTimer(){ //슬라이더 조정
        self.timer = Timer.scheduledTimer(withTimeInterval: 0.01, repeats: true, block: { [unowned self] (timer: Timer) in
          
            if self.progressSlider.isTracking { return }
            
            self.updateTimeLabelText(time: self.player.currentTime) //텍스트 라벨 함수 호출 후 그 시간에 맞게 설정
            self.progressSlider.value = Float(self.player.currentTime) // 슬라이더
        })
        self.timer.fire()
    }
    func invalidateTimer() { //타이머 초기화?
